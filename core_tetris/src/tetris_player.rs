...
use super::{
    BagType, CircularBuffer, CoreTetrisError, TetrisCommand, TetrisGrid, TetrisResult, Tetromino,
...
impl TetrisPlayer {
    ...
    pub fn default<R: Rng>(rng: &mut R, bag_type: BagType) -> Self {
        TetrisPlayer::try_from_matrix(rng, bag_type, TetrisGrid::default())
            .expect("TetrisPlayer::try_from_matrix shouldn't fail for an empty grid")
    }
    ...
    pub fn try_from_matrix<R: Rng>(
        ...
        let next_queue = CircularBuffer::try_new(next_tetrominos)?;
